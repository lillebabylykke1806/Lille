...
export default function Amming({ bruker }: Props) {
  ...
  const anbefalingBryst = logg.length > 0 ? motattBryst(logg[0].bryst) : 'høyre';
    ...
    <div style={{ backgroundColor: farger.bakgrunn, minHeight: '100vh', padding: '24px 24px 100px' }}>
      ...
      {aktiv && (
        <div style={{ backgroundColor: farger.hvit, border: `1px solid ${farger.kremMørk}`, borderRadius: '20px', padding: '28px 20px', marginBottom: '16px', textAlign: 'center' }}>
          ...
          <div style={{ fontSize: '22px', fontFamily: 'var(--font-plus-jakarta)', color: farger.tekst, fontWeight: '700', marginBottom: '6px', animation: byttAnimasjon ? 'bytt 0.4s ease' : 'none' }}>
      ...
      {!aktiv && (
        <>
          ...
          {/* Start amming - velg bryst direkte */}
          <div style={{ backgroundColor: farger.hvit, border: `1px solid ${farger.kremMørk}`, borderRadius: '20px', padding: '20px', marginBottom: '12px' }}>
            <div style={{ fontSize: '14px', fontFamily: 'var(--font-plus-jakarta)', color: farger.tekst, fontWeight: '600', marginBottom: '14px', textAlign: 'center' }}>
              {logg.length > 0 ? 'Start amming' : 'Velg bryst for å starte'}
            </div>
            <div style={{ display: 'flex', gap: '12px' }}>
              <button
                onClick={() => startAmming('venstre')}
                style={{ flex: 1, padding: '20px 12px', backgroundColor: anbefalingBryst === 'venstre' ? farger.grønnLys : farger.bakgrunn, border: `1.5px solid ${anbefalingBryst === 'venstre' ? farger.grønn : farger.kremMørk}`, borderRadius: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px', cursor: 'pointer' }}
              >
                ...
                <div style={{ fontSize: '14px', fontFamily: 'var(--font-inter)', color: anbefalingBryst === 'venstre' ? farger.grønn : farger.tekst, fontWeight: anbefalingBryst === 'venstre' ? '600' : '500' }}>
                  Venstre
                </div>
                {anbefalingBryst === 'venstre' && (
                  <div style={{ fontSize: '10px', fontFamily: 'var(--font-inter)', color: farger.grønn }}>Anbefalt</div>
                )}
              </button>
              <button
                onClick={() => startAmming('høyre')}
                style={{ flex: 1, padding: '20px 12px', backgroundColor: anbefalingBryst === 'høyre' ? farger.grønnLys : farger.bakgrunn, border: `1.5px solid ${anbefalingBryst === 'høyre' ? farger.grønn : farger.kremMørk}`, borderRadius: '16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '10px', cursor: 'pointer' }}
              >
                <div style={{ width: '52px', height: '52px', borderRadius: '50%', backgroundColor: '#FFE8D6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <img src="/tateflaske-mork.png" style={{ width: 28, height: 28, objectFit: 'contain' }} />
                </div>
                <div style={{ fontSize: '14px', fontFamily: 'var(--font-inter)', color: anbefalingBryst === 'høyre' ? farger.grønn : farger.tekst, fontWeight: anbefalingBryst === 'høyre' ? '600' : '500' }}>
                  Høyre
                </div>
                {anbefalingBryst === 'høyre' && (
                  <div style={{ fontSize: '10px', fontFamily: 'var(--font-inter)', color: farger.grønn }}>Anbefalt</div>
                )}
              </button>
            </div>
