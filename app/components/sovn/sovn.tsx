...
import { formatTime, formatTimeValue } from '../../lib/i18n/format';
...
const tilTidsformat = (tid: string): string => {
  if (!tid) return '';
  return tid.replace(/\./g, ':').slice(0, 5); // Tar bare HH:MM
};
...
Svar KUN med observasjonen, ingen introduksjon.`
  ...
  const justerStartTidManuelt = async () => {
    if (!nyTidStr) return;
    const normalized = nyTidStr.replace(/\./g, ':').slice(0, 5);
    const [h, m] = normalized.split(':').map(Number);
    if (!Number.isFinite(h) || !Number.isFinite(m)) return;
    const nyTid = new Date(`${nyDatoJuster}T00:00:00`);
    nyTid.setHours(h, m, 0, 0);
    if (Number.isNaN(nyTid.getTime()) || nyTid > new Date()) return;
    setStartTid(nyTid);
    localStorage.setItem('lille_starttid', nyTid.toISOString());
    // Persist immediately — stoppSøvn used to only write slutt, which left start as "now"
    if (lurId) {
      await supabase.from('lurer').update({
        start: formatTimeValue(nyTid),
        dato: nyDatoJuster,
      }).eq('id', lurId);
    }
  ...
  const stoppSøvn = async () => {
    ...
    const varighetMinutter = Math.max(0, Math.floor(diff / 60));
    const startStr = formatTimeValue(startTid);
    const sluttStr = formatTimeValue(slutt);
    const startDato = `${startTid.getFullYear()}-${String(startTid.getMonth() + 1).padStart(2, '0')}-${String(startTid.getDate()).padStart(2, '0')}`;
    if (lurId) {
      await supabase.from('lurer').update({
        start: startStr,
        dato: startDato,
  ...
  const lagreEtterregistrert = async () => {
    ...
    await supabase.from('lurer').insert({
      ...
      start: tilTidsformat(nyStart), slutt: nySlutt ? tilTidsformat(nySlutt) : null, varighet, signaler: '',
    });
  ...
  <div style={{ flex: 1, backgroundColor: farger.bakgrunn, borderRadius: '14px', padding: '14px', textAlign: 'center' }}>
    ...
    <div style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.tekstLys }}>{t('søvn.lengsteStrekk')}</div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', animation: 'fadeOpp 0.8s 0.7s ease-out both' }}>
          ...
          <button onClick={() => { setNyTidStr(formatTimeValue(new Date())); setNyDatoJuster(startTid ? startTid.toISOString().split('T')[0] : dagensdato()); setVisJusterTid(true); }} style={{ background: 'none', border: 'none', fontSize: '12px', fontFamily: 'var(--font-inter)', color: farger.tekstLys, cursor: 'pointer', textDecoration: 'underline', padding: '4px' }}>
            ...
            <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={() => setVisJusterTid(false)}>
              <div onClick={e => e.stopPropagation()} style={{ backgroundColor: farger.hvit, width: '100%', maxWidth: '430px', borderRadius: '24px 24px 0 0', padding: '24px', paddingBottom: '48px' }}>
                ...
                <button onClick={async () => {
                  const profilId = await hentProfilId(aktivtBarn, bruker);
                  if (!profilId) return;
                  await supabase.from('lurer').insert({ profil_id: profilId, dato: dagensdato(), type: 'oppvåkning', start: tilTidsformat(nyTidStr), slutt: null, varighet: 0, signaler: '' });
  ...
  if (visning === 'velg') {
    return (
      <div style={{ padding: '24px', textAlign: 'center' }}>
        ...
        <button onClick={() => setVisning('etterregistrer')} style={{ width: '100%', padding: '14px 20px', backgroundColor: farger.grønnLys, border: `1px solid ${farger.grønn}`, borderRadius: '16px', display: 'flex', alignItems: 'center', gap: '16px', cursor: 'pointer', textAlign: 'left', marginBottom: '16px' }}>
          <svg width="36" height="36" viewBox="0 0 36 36" fill="none">
            <circle cx="18" cy="18" r="14" stroke={farger.grønn} strokeWidth="1.5" fill="none"/>
            <line x1="18" y1="11" x2="18" y2="18" stroke={farger.grønn} strokeWidth="1.5" strokeLinecap="round"/>
            <line x1="18" y1="18" x2="23" y2="21" stroke={farger.grønn} strokeWidth="1.5" strokeLinecap="round"/>
          </svg>
          <div>
            <div style={{ fontSize: '16px', fontFamily: 'var(--font-plus-jakarta)', color: farger.grønn, marginBottom: '2px', fontWeight: '400' }}>{t('søvn.etterregistrer')}</div>
            <div style={{ fontSize: '13px', fontFamily: 'var(--font-inter)', color: farger.grønn }}>{t('søvn.etterregistrerBeskrivelse')}</div>
          </div>
          <div style={{ marginLeft: 'auto', color: farger.grønn, fontSize: '18px' }}>›</div>
        </button>
          ...
          {lurFerdig ? (
            ...
            <>
              <div style={{ position: 'relative', display: 'flex', justifyContent: 'center', alignItems: 'center', marginBottom: '16px', minHeight: '260px', zIndex: 1 }}>
                <button onClick={() => { setNyTidStr(startTid ? formatTimeValue(startTid) : ''); setNyDatoJuster(startTid ? startTid.toISOString().split('T')[0] : dagensdato()); setVisJusterTid(!visJusterTid); }} style={{ background: 'none', border: 'none', cursor: 'pointer', position: 'relative', width: '220px', height: '220px' }}>
              ...
              <div style={{ backgroundColor: farger.hvit, border: `1px solid ${farger.kremMørk}`, borderRadius: '16px', padding: '16px', marginBottom: '12px' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '10px' }}>
                  {signaler.map(signal => (
                    <button key={signal.id} onClick={() => toggleSignal(signal.id)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: '16px 8px', borderRadius: '16px', border: valgteSignaler.includes(signal.id) ? `1.5px solid ${farger.grønn}` : `1px solid ${farger.kremMørk}`, backgroundColor: valgteSignaler.includes(signal.id) ? `${farger.grønn}22` : farger.bakgrunn, cursor: 'pointer' }}>
                      ...
                      <div style={{ fontSize: '13px', fontFamily: 'var(--font-inter)', color: valgteSignaler.includes(signal.id) ? farger.grønn : farger.tekstLys, textAlign: 'center', lineHeight: 1.2 }}>{valgteSignaler.includes(signal.id) ? `✓ ${signal.label}` : signal.label}</div>
                    </button>
                  ))}
                </div>
...
{tidslinje.length > 0 && (
  ...
  <button onClick={() => { setNyTidStr(startTid ? formatTimeValue(startTid) : ''); setNyDatoJuster(startTid ? startTid.toISOString().split('T')[0] : dagensdato()); setVisJusterTid(!visJusterTid); }} style={{ background: 'none', border: 'none', cursor: 'pointer', marginTop: '6px' }}>
