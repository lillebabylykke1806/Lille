...
const getTiltak = (t: TFn) => [
  ...
  { id: 'Bæresjal', label: t('kolikk.tiltakBæresjal') },
  { id: 'Amming', label: t('kolikk.tiltakAmming') },
  { id: 'Flaske', label: t('kolikk.tiltakFlaske') },
  { id: 'Hud mot hud', label: t('kolikk.tiltakHudMotHud') },
  { id: 'Mørkt rom', label: t('kolikk.tiltakMørktRom') },
  { id: 'Vugging', label: t('kolikk.tiltakVugging') },
  { id: 'Hvit støy', label: t('kolikk.tiltakHvitStøy') },
  { id: 'Smokk', label: t('kolikk.tiltakSmokk') },
];

const getSignalerUro = (t: TFn) => [
  { id: 'Trekker bena opp', label: t('kolikk.signalTrekkerBenaOpp') },
  { id: 'Bøyer ryggen', label: t('kolikk.signalBøyerRyggen') },
  { id: 'Vrir seg', label: t('kolikk.signalVrirSeg') },
  { id: 'Klynker', label: t('kolikk.signalKlynker') },
  { id: 'Høy gråt', label: t('kolikk.signalHøyGråt') },
  { id: 'Vil bæres', label: t('kolikk.signalVilBæres') },
  { id: 'Vanskelig å legge ned', label: t('kolikk.signalVanskeligÅLeggeNed') },
  { id: 'Mye luft', label: t('kolikk.signalMyeLuft') },
  { id: 'Slår med armer og ben', label: t('kolikk.signalSlårMedArmer') },
];

export default function Kolikk({ bruker, aktivtBarn }: Props) {
  const { t, locale } = useLanguage();
  const TILTAK = getTiltak(t);
  const SIGNALER_URO = getSignalerUro(t);
  const finnTiltakLabel = (id: string) => TILTAK.find(item => item.id === id)?.label ?? id;
  const finnSignalLabel = (id: string) => SIGNALER_URO.find(item => item.id === id)?.label ?? id;
...
const [visAlleTiltak, setVisAlleTiltak] = useState(false);
  ...
  const hentAIMønstre = useCallback(async () => {
    ...
    try {
      const response = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: 'user',
            content: `Du er en varm babyekspert i appen Lille. Analyser disse uro-registreringene for ${babyNavn} og gi 3 korte mønstre/observasjoner på ${språkNavn}. Start hver med ✨. Maks 1 setning per punkt. Vær konkret og handlingsrettet.
...
Data: ${JSON.stringify(logg.slice(0, 10))}`
  ...
  const innsiktKort = [
    {
      ...
      bg: '#F0EBF8',
      label: t('kolikk.innsiktNårUroen'),
    },
    {
      icon: (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      bg: '#FFF0F0',
      label: t('kolikk.innsiktTegnFørUro'),
    },
    {
      icon: (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      bg: '#FFFBEC',
      label: t('kolikk.innsiktTiltakSomKanHjelpe'),
    },
    {
      icon: (
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
      ...
      bg: '#E8F0E8',
      label: t('kolikk.innsiktMønstreOverTid'),
    },
  ];

  const komIGangSteg = [
    t('kolikk.stegRegistrer'),
    t('kolikk.stegLeggTilSignaler'),
    t('kolikk.stegNoterHvaDere'),
  ];
  ...
  return (
    <div style={{ backgroundColor: farger.bakgrunn, minHeight: '100vh', padding: '0 0 100px' }}>
      ...
      <div style={{ padding: '24px 24px 16px', textAlign: 'center' }}>
        <div style={{ fontSize: '26px', fontFamily: 'var(--font-plus-jakarta)', color: farger.tekst, fontWeight: '700', marginBottom: '4px' }}>{t('kolikk.tittel')}</div>
        <div style={{ fontSize: '13px', fontFamily: 'var(--font-inter)', color: farger.tekstLys }}>{t('kolikk.undertittel')}</div>
      </div>
      ...
      <div style={{ padding: '0 16px' }}>
        ...
        {nesteUro && parseInt(nesteUro.om.replace(/\D/g, '')) <= 60 && (
          <div style={{ backgroundColor: '#FFF3EE', border: '1px solid #F4C4A8', borderRadius: '20px', padding: '20px', marginBottom: '12px', display: 'flex', alignItems: 'center', gap: '16px' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: '#C48E7B', fontWeight: '600', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>{t('kolikk.nåAkkurat')}</div>
              <div style={{ fontSize: '17px', fontFamily: 'var(--font-plus-jakarta)', color: farger.tekst, fontWeight: '700', marginBottom: '4px' }}>{t('kolikk.påVeiInnIUroperiode')}</div>
              <div style={{ fontSize: '13px', fontFamily: 'var(--font-inter)', color: farger.tekstLys, lineHeight: 1.5 }}>{t('kolikk.påVeiInnBeskrivelse', { navn: babyNavn })}</div>
            </div>
          </div>
        )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
            ...
            <div style={{ backgroundColor: farger.hvit, border: `1px solid ${farger.kremMørk}`, borderRadius: '20px', padding: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '8px' }}>
                ...
                <div style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.grønn, fontWeight: '600' }}>{t('kolikk.nesteForventedeUro')}</div>
              </div>
              ...
              <div style={{ fontSize: '12px', fontFamily: 'var(--font-inter)', color: farger.tekstLys, marginBottom: '12px' }}>{t('kolikk.vanligStart', { tid: nesteUro?.tidspunkt ?? '—' })}</div>
              <button onClick={() => setVarslingPå(!varslingPå)} style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '8px', backgroundColor: farger.bakgrunn, borderRadius: '10px', border: 'none', cursor: 'pointer', width: '100%' }}>
                <div style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.tekstLys, flex: 1, textAlign: 'left' }}>{t('kolikk.varsleMe45min')}</div>
            ...
            <div style={{ backgroundColor: farger.hvit, border: `1px solid ${farger.kremMørk}`, borderRadius: '20px', padding: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '10px' }}>
                ...
                <div style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.grønn, fontWeight: '600' }}>{t('kolikk.vanligeSignalerFørUro')}</div>
              </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {signalStatistikk.map((s, i) => (
                    <div key={s.signal} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      <div style={{ flex: 1, fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.tekst, lineHeight: 1.2 }}>{finnSignalLabel(s.signal)}</div>
  ...
  {t('kolikk.seSignalhistorikk')}
</button>
                </div>
              ) : (
                <div style={{ fontSize: '12px', fontFamily: 'var(--font-inter)', color: farger.tekstLys }}>{t('kolikk.registrerEpisoder')}</div>
              )}
            </div>
          </div>
        )}
        ...
        {logg.length >= 1 && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
            ...
            <div style={{ backgroundColor: farger.hvit, border: `1px solid ${farger.kremMørk}`, borderRadius: '20px', padding: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '10px' }}>
                <div style={{ fontSize: '14px' }}>💆</div>
                <div style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.grønn, fontWeight: '600' }}>{t('kolikk.tiltakSomHjelper')}</div>
              </div>
              {tiltakStatistikk.slice(0, 3).map((stat, i) => (
                <div key={stat.tiltak} style={{ marginBottom: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '3px' }}>
                    <div style={{ fontSize: '12px' }}>{medalje(i)}</div>
                    <div style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.tekst, flex: 1 }}>{finnTiltakLabel(stat.tiltak)}</div>
                    <div style={{ fontSize: '10px', fontFamily: 'var(--font-inter)', color: farger.tekstLys }}>{stat.fungerte}/{stat.total}</div>
                  </div>
                  <div style={{ height: '4px', backgroundColor: farger.bakgrunn, borderRadius: '2px', overflow: 'hidden' }}>
                    <div style={{ height: '100%', width: `${Math.round((stat.fungerte / stat.total) * 100)}%`, backgroundColor: farger.grønn, borderRadius: '2px' }} />
                  </div>
                </div>
              ))}
  ...
  {t('kolikk.seAlleTiltak')}
            ...
            <div style={{ backgroundColor: farger.hvit, border: `1px solid ${farger.kremMørk}`, borderRadius: '20px', padding: '16px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '10px' }}>
                <div style={{ fontSize: '14px' }}>✨</div>
                <div style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.grønn, fontWeight: '600' }}>{t('kolikk.mønsterViSer')}</div>
              </div>
              {lasterAI ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  ...
                  <button style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.grønn, padding: 0, marginTop: '4px' }}>
                    {t('kolikk.seAlleInnsikter')}
                  </button>
                </div>
              ) : (
                <div style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.tekstLys }}>{t('kolikk.registrerEpisoder')}</div>
              )}
            </div>
          </div>
        )}
                ...
                return (
                  <div key={l.id} style={{ backgroundColor: farger.hvit, border: `1px solid ${farger.kremMørk}`, borderRadius: '16px', padding: '16px', animation: 'fadeOpp 0.4s ease' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px', paddingBottom: '12px', borderBottom: `1px solid ${farger.bakgrunn}` }}>
                      ...
                      {l.varighet && (
                        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '4px', fontSize: '12px', fontFamily: 'var(--font-inter)', color: farger.tekstLys }}>
                          {t('kolikk.varighet', { min: l.varighet })}
                        </div>
                      )}
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '8px' }}>
                      <div>
                        <div style={{ fontSize: '10px', fontFamily: 'var(--font-inter)', color: farger.tekstLys, fontWeight: '600', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{t('kolikk.signaler')}</div>
                        {l.signaler.slice(0, 3).map((s, i) => (
                          <div key={i} style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.tekst, marginBottom: '2px' }}>• {finnSignalLabel(s)}</div>
                        ))}
                      </div>
                      <div>
                        <div style={{ fontSize: '10px', fontFamily: 'var(--font-inter)', color: farger.tekstLys, fontWeight: '600', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{t('kolikk.tiltak')}</div>
                        {l.tiltak.slice(0, 3).map((tiltakId, i) => (
                          <div key={i} style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.tekst, marginBottom: '2px' }}>• {finnTiltakLabel(tiltakId)}</div>
                        ))}
                      </div>
                      <div style={{ textAlign: 'center' }}>
                        <div style={{ fontSize: '10px', fontFamily: 'var(--font-inter)', color: farger.tekstLys, fontWeight: '600', marginBottom: '4px', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{t('kolikk.resultat')}</div>
                        <div style={{ fontSize: '24px' }}>{l.resultat === 'bra' ? '😊' : l.resultat === 'delvis' ? '😐' : l.resultat === 'ikke' ? '😔' : '—'}</div>
                        {l.resultat === 'bra' && l.varighet && (
                          <div style={{ fontSize: '10px', fontFamily: 'var(--font-inter)', color: farger.grønn, lineHeight: 1.3 }}>{t('kolikk.roetSegEtter', { min: l.varighet })}</div>
                        )}
                        {l.resultat === 'delvis' && (
                          <div style={{ fontSize: '10px', fontFamily: 'var(--font-inter)', color: farger.tekstLys }}>{t('kolikk.delvisEffekt')}</div>
                        )}
                      </div>
                    </div>
                  </div>
...
{logg.length === 0 && (
  ...
  <img src="/sky.png" alt="sky" style={{ position: 'relative', zIndex: 1, width: '100%', maxWidth: '400px', height: 'auto', objectFit: 'contain' }} />
      ...
      <button onClick={() => setVisRegistrer(true)} style={{ width: '100%', padding: '16px', backgroundColor: farger.grønn, border: 'none', borderRadius: '16px', fontSize: '15px', fontWeight: '600', color: '#FDFAF6', cursor: 'pointer', fontFamily: 'var(--font-inter)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
        ...
        {t('kolikk.registrerFørsteUroperiode')}
      </button>
    </div>
    ...
    <div style={{ backgroundColor: farger.hvit, border: `1px solid ${farger.kremMørk}`, borderRadius: '20px', padding: '20px', marginBottom: '12px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
        ...
        <div>
          <div style={{ fontSize: '14px', fontFamily: 'var(--font-plus-jakarta)', color: farger.tekst, fontWeight: '600' }}>{t('kolikk.komIGang')}</div>
          <div style={{ fontSize: '12px', fontFamily: 'var(--font-inter)', color: farger.tekstLys }}>{t('kolikk.komIGangBeskrivelse', { navn: babyNavn })}</div>
        </div>
      </div>
      {komIGangSteg.map((punkt, i) => (
      ...
      {visRegistrer && (
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={() => setVisRegistrer(false)}>
          <div onClick={e => e.stopPropagation()} style={{ backgroundColor: farger.hvit, width: '100%', maxWidth: '430px', borderRadius: '24px 24px 0 0', padding: '24px', paddingBottom: '48px', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ fontSize: '18px', fontFamily: 'var(--font-plus-jakarta)', color: farger.tekst, fontWeight: '600', marginBottom: '20px' }}>{t('kolikk.registrerUroEpisode')}</div>

            <div style={{ marginBottom: '16px' }}>
              <div style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--font-inter)', color: farger.tekstLys, marginBottom: '8px' }}>{t('kolikk.tidspunkt')}</div>
            ...
            <div style={{ marginBottom: '16px' }}>
              <div style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--font-inter)', color: farger.tekstLys, marginBottom: '8px' }}>{t('kolikk.hvaSerDuNå')}</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {SIGNALER_URO.map(s => (
                  <button key={s.id} onClick={() => setNySignaler(prev => prev.includes(s.id) ? prev.filter(x => x !== s.id) : [...prev, s.id])}
                    style={{ padding: '8px 14px', borderRadius: '20px', border: `1.5px solid ${nySignaler.includes(s.id) ? farger.grønn : farger.kremMørk}`, backgroundColor: nySignaler.includes(s.id) ? farger.grønnLys : farger.bakgrunn, color: nySignaler.includes(s.id) ? farger.grønn : farger.tekst, fontSize: '13px', fontFamily: 'var(--font-inter)', cursor: 'pointer', fontWeight: nySignaler.includes(s.id) ? '600' : '400' }}>
                    {s.label}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: '16px' }}>
              <div style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--font-inter)', color: farger.tekstLys, marginBottom: '8px' }}>{t('kolikk.hvaPrøvdeDere')}</div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px' }}>
                {TILTAK.map(tiltakItem => (
                  <button key={tiltakItem.id} onClick={() => setNyTiltak(prev => prev.includes(tiltakItem.id) ? prev.filter(x => x !== tiltakItem.id) : [...prev, tiltakItem.id])}
                    style={{ padding: '8px 14px', borderRadius: '20px', border: `1.5px solid ${nyTiltak.includes(tiltakItem.id) ? farger.grønn : farger.kremMørk}`, backgroundColor: nyTiltak.includes(tiltakItem.id) ? farger.grønnLys : farger.bakgrunn, color: nyTiltak.includes(tiltakItem.id) ? farger.grønn : farger.tekst, fontSize: '13px', fontFamily: 'var(--font-inter)', cursor: 'pointer', fontWeight: nyTiltak.includes(tiltakItem.id) ? '600' : '400' }}>
                    {tiltakItem.label}
                  </button>
                ))}
              </div>
            ...
            <div style={{ marginBottom: '16px' }}>
              <div style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--font-inter)', color: farger.tekstLys, marginBottom: '8px' }}>{t('kolikk.hvordanGikkDet')}</div>
            ...
            <div style={{ marginBottom: '16px' }}>
              <div style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--font-inter)', color: farger.tekstLys, marginBottom: '8px' }}>{t('kolikk.varighetMinutter')}</div>
              <input type="number" value={nyVarighet} onChange={e => setNyVarighet(e.target.value)} placeholder={t('kolikk.varighetPlaceholder')} style={{ width: '100%', padding: '12px 16px', fontSize: '14px', border: `1px solid ${farger.kremMørk}`, borderRadius: '12px', backgroundColor: farger.bakgrunn, color: farger.tekst, outline: 'none', fontFamily: 'var(--font-inter)', boxSizing: 'border-box' }} />
            </div>

            <div style={{ marginBottom: '24px' }}>
              <div style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: 'var(--font-inter)', color: farger.tekstLys, marginBottom: '8px' }}>{t('kolikk.notatValgfritt')}</div>
              <textarea value={nyNotat} onChange={e => setNyNotat(e.target.value)} placeholder={t('kolikk.notatPlaceholder')} style={{ width: '100%', padding: '12px 16px', fontSize: '14px', border: `1px solid ${farger.kremMørk}`, borderRadius: '12px', backgroundColor: farger.bakgrunn, color: farger.tekst, outline: 'none', fontFamily: 'var(--font-inter)', resize: 'none', minHeight: '70px', boxSizing: 'border-box' }} />
            </div>
            ...
            <button onClick={lagreUro} disabled={lagrer} style={{ width: '100%', padding: '16px', backgroundColor: farger.grønnLys, border: `1px solid ${farger.grønn}`, borderRadius: '16px', fontSize: '15px', fontWeight: '600', color: farger.grønn, cursor: 'pointer', fontFamily: 'var(--font-inter)' }}>
              {lagrer ? t('kolikk.lagrer') : t('kolikk.lagreEpisode')}
            </button>
          </div>
        </div>
      )}
...
{visSignalHistorikk && (
  <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={() => setVisSignalHistorikk(false)}>
    <div onClick={e => e.stopPropagation()} style={{ backgroundColor: farger.hvit, width: '100%', maxWidth: '430px', borderRadius: '24px 24px 0 0', padding: '24px', paddingBottom: '48px', maxHeight: '85vh', overflowY: 'auto' }}>
      <div style={{ width: '36px', height: '4px', backgroundColor: farger.kremMørk, borderRadius: '2px', margin: '0 auto 20px' }} />
      <div style={{ fontSize: '18px', fontFamily: 'var(--font-plus-jakarta)', color: farger.tekst, fontWeight: '700', marginBottom: '20px' }}>{t('kolikk.signalhistorikk')}</div>
      {logg.flatMap(l => l.signaler).length === 0 ? (
        <div style={{ fontSize: '13px', fontFamily: 'var(--font-inter)', color: farger.tekstLys, textAlign: 'center', padding: '24px' }}>{t('kolikk.ingenSignalerRegistrert')}</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
          {Object.entries(
            ...
            <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '12px 16px', backgroundColor: farger.bakgrunn, borderRadius: '14px' }}>
              ...
              <div style={{ flex: 1, fontSize: '14px', fontFamily: 'var(--font-inter)', color: farger.tekst }}>{finnSignalLabel(signal)}</div>
...
{visAlleTiltak && (
  <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.4)', zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }} onClick={() => setVisAlleTiltak(false)}>
    <div onClick={e => e.stopPropagation()} style={{ backgroundColor: farger.hvit, width: '100%', maxWidth: '430px', borderRadius: '24px 24px 0 0', padding: '24px', paddingBottom: '48px', maxHeight: '85vh', overflowY: 'auto' }}>
      <div style={{ width: '36px', height: '4px', backgroundColor: farger.kremMørk, borderRadius: '2px', margin: '0 auto 20px' }} />
      <div style={{ fontSize: '18px', fontFamily: 'var(--font-plus-jakarta)', color: farger.tekst, fontWeight: '700', marginBottom: '4px' }}>{t('kolikk.alleTiltak')}</div>
      <div style={{ fontSize: '12px', fontFamily: 'var(--font-inter)', color: farger.tekstLys, marginBottom: '20px' }}>{t('kolikk.rangertEtterEffekt')}</div>
      {tiltakStatistikk.length === 0 ? (
        <div style={{ fontSize: '13px', fontFamily: 'var(--font-inter)', color: farger.tekstLys, textAlign: 'center', padding: '24px' }}>{t('kolikk.ingenTiltakRegistrert')}</div>
      ) : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
          {tiltakStatistikk.map((stat, i) => (
            <div key={stat.tiltak} style={{ padding: '14px 16px', backgroundColor: farger.bakgrunn, borderRadius: '14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '8px' }}>
                <div style={{ fontSize: '16px' }}>{medalje(i)}</div>
                <div style={{ flex: 1, fontSize: '14px', fontFamily: 'var(--font-inter)', color: farger.tekst, fontWeight: '600' }}>{finnTiltakLabel(stat.tiltak)}</div>
                <div style={{ fontSize: '12px', fontFamily: 'var(--font-inter)', color: farger.tekstLys }}>{stat.fungerte}/{stat.total} {t('kolikk.ganger', { antall: stat.total }).replace(String(stat.total), '').trim()}</div>
              </div>
              <div style={{ height: '6px', backgroundColor: farger.kremMørk, borderRadius: '3px', overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${Math.round((stat.fungerte / stat.total) * 100)}%`, backgroundColor: farger.grønn, borderRadius: '3px' }} />
              </div>
              <div style={{ fontSize: '11px', fontFamily: 'var(--font-inter)', color: farger.grønn, fontWeight: '600', marginTop: '4px' }}>
                {t('kolikk.prosent', { pst: Math.round((stat.fungerte / stat.total) * 100) })}
              </div>
            </div>
          ))}
        </div>
